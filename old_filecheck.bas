...
download_path = "https://raw.github.com/domso/noname/master/"
	...
	Do
		...
		If tmp <> 0 Then
			...
			Do
				...
				If tmp2 <> 0 Then
					If tmp2->file_name = tmp->file_name Then Exit do
				EndIf
			Loop Until tmp2 = 0
			...
			End If
			logMSG("start download: "+download_path + Mid(tmp->path,3) + tmp->file_name)
			If download(download_path + Mid(tmp->path,3) + tmp->file_name,tmp->path + tmp->file_name)=1 Then
